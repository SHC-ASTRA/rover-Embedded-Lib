...
void AstraArm::IK_Execute() {
    ...
    if (this->ik_obj.active) {
        this->fabrik.setJoints(this->angles,
                               this->segments);   // Forward kinematics, get end effector position
        ...
        if (updateJointSpeeds() == 1)
            this->ik_obj.active =
                false;  // All joints are at their target angles, so the IK objective is complete
    }
};
...
int AstraArm::IK_Plan(float rel_target_x, float rel_target_y) {
    ...
    this->ik_output =
        this->fabrik.solve(this->ik_obj.target_pos[0], this->ik_obj.target_pos[1], this->segments);
    if (this->ik_output == 0)
    ...
    // UPDATE: This might not need to happen, since the motor speeds will be updated right after run
    // with IK_Execute()
};

int AstraArm::updateJointSpeeds()  // Returns 1 if all joints are at their target angles, 0
                                   // otherwise
{
    int goals_reached = 0;
    ...
    for (int i = 2; i >= 0; i--) {
        ...
        if ((this->angles[i] > (this->ik_obj.target_angles[i] - 2.0)) &&
            (this->angles[i] < (this->ik_obj.target_angles[i] +
                                2.0)))  // if the joint is within 2 degrees of the target angle
        {
            this->ik_obj.motor_speeds[i] =
                0;  // stop the motor once it reaches within 2 degrees of the target angle
            goals_reached++;
            continue;  // go to the next joint
        }

        // for this part need to do some conversions to prevent overshooting the position
        float converted_target =
            this->ik_obj.target_angles[i] + 180.0;  // convert target angle to 0-360 degrees
        ...
        if (this->ik_obj.motor_speeds[i] < 0)  // CCW
        {
            if (this->angles[i] > converted_target)  // if the joint is past the target angle
            {
                ...
                // this->angles[i] = this->ik_obj.target_angles[i];//set the joint angle to the
                // target angle goals_reached++;
                continue;  // go to the next joint
            }
        } else  // CW
        {
        ...
        if (i == 2)  // if axis 3
        {
            ...
            if (this->ik_obj.target_angles[i] >
                converted_target)  // if the target angle is greater than the current angle
            {
            ...
            if (i == 1)  // axis 2
            {
                ...
                this->ik_obj.motor_speeds[i] =
                    (((abs(this->angles[i] - this->ik_obj.target_angles[i]) + ax_2_k2) / ax_2_k1)) /
                    100.0;
            } else {  // axis 1
                this->ik_obj.motor_speeds[i] =
                    (((abs(this->angles[i] - this->ik_obj.target_angles[i]) + ax_1_k2) / ax_1_k1)) /
                    100.0;
            }
        }
    }

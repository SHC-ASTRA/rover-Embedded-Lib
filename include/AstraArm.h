...
class AstraArm {
   public:
    ...
    void IK_Execute();  // Execute the inverse kinematics for the arm (update arm
                        // speeds,angles,pos,etc..)
    int IK_Plan(
        float rel_target_x,
        float rel_target_y);  // Plan the inverse kinematics for the arm (based on the set
                              // objective). This should run when new control commands are received.
